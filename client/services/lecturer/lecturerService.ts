...
export async function deleteExamWithAuth(
  examId: number,
  username: string,
  password: string
) {
  ...
  if (!res.ok) {
    const error = await res.json().catch(() => ({}));
    throw new Error(
      error?.error || 'Invalid credentials or failed to delete exam'
    );
  }
...
export async function fetchExamResults(examId: number) {
  const res = await fetch(`${url}/lecturer/exams/${examId}/results`);
